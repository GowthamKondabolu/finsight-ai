...
def make_company_facts_result() -> CompanyFactsIngestionResult:
    """Create a deterministic company-facts CLI result."""

    return CompanyFactsIngestionResult(
        cik="0000320193",
        company_id=COMPANY_ID,
        discovered_observations=100,
        selected_observations=80,
...
@pytest.mark.asyncio
async def test_run_company_facts_ingestion_releases_resources(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Company-facts CLI execution should release HTTP and database resources."""
    ...
    client_context.__aenter__ = AsyncMock(return_value=client)
    client_context.__aexit__ = AsyncMock(return_value=None)
    ingestion = AsyncMock(return_value=make_company_facts_result())
    ...
    monkeypatch.setattr(cli_module, "ingest_company_facts", ingestion)

    result = await cli_module.run_company_facts_ingestion(
        cik="0000320193",
        taxonomies={"us-gaap"},
    )

    assert result == make_company_facts_result()
    ingestion.assert_awaited_once_with(
        client=client,
        session_factory=session_factory,
        cik="0000320193",
...
@pytest.mark.parametrize(
    ("arguments", "expected_taxonomies"),
    [
        (
            ["ingest-company-facts", "--cik", "320193"],
            DEFAULT_COMPANY_FACT_TAXONOMIES,
        ),
        (
            [
...
def test_main_runs_company_facts_ingestion(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
    arguments: list[str],
    expected_taxonomies: Collection[str],
) -> None:
    """The CLI should support default and repeated taxonomy selection."""

    ingestion = AsyncMock(return_value=make_company_facts_result())
    monkeypatch.setattr(cli_module, "run_company_facts_ingestion", ingestion)
    ...
    ingestion.assert_awaited_once_with(
        cik="320193",
        taxonomies=expected_taxonomies,
    )
    payload = json.loads(capsys.readouterr().out)
